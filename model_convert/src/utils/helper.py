...
def load_onnx_model(model_type):
    if model_type == 'appearance_feature_extractor':
        model = ort.InferenceSession("export/feature_extractor.onnx", providers=["CPUExecutionProvider"])
    elif model_type == 'motion_extractor':
        model = ort.InferenceSession('export/motion_extractor.onnx', providers=["CPUExecutionProvider"])
    elif model_type == 'warping_module':
        model = ort.InferenceSession('export/warp.onnx', providers=["CPUExecutionProvider"])
    elif model_type == 'spade_generator':
        model = ort.InferenceSession('export/spade_generator.onnx', providers=["CPUExecutionProvider"])
    elif model_type == 'stitching_retargeting_module':
        model = ort.InferenceSession('export/stitching_retargeting.onnx', providers=["CPUExecutionProvider"])
    return model

def load_model(ckpt_path, model_config, device, model_type, onnx_inference):
    if onnx_inference:
        model = load_onnx_model(model_type)
        return model
